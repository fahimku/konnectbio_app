...
function Gallery({ title, getMedia, gallery, deleteMedia, schedulePost,getCategories,categories}) {
    ...
    const onDelete = async (item) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                deleteMedia(item.media_library_id)
                    .then(() => {
                        getMedia()
            ...
            return (
                <Row className="post-analytics-tab-boxes-ift">
                    {gallery.length > 0 ? (
                                ...
                                >
                                    <div className="camp-row row">
                                        <div className="campaign-header col-12">
                                            <h6>{item.title.length>20?item.title.slice(0,20)+"...":item.title}</h6>
                                        </div>
                                        <div className="any-post-img-col col-12" style={{ marginBottom: 50 }}>
                        ...
                        <div style={{ height: 300,width:'100%', display: 'flex', flexDirection: 'row', alignItems: 'center', justifyContent: 'center' }}>
                    ...
                    <Col lg={12}>
                        <div>
                            ...
                            <Input
                                ...
                                onChange={(e)=>setFields({...fields,redirected_url:e.target.value})}
                            />
    ...
    function onSubmit(){
        ...
        if(showPost){
            if(showBio){
                if(fields.publish_date && fields.redirected_url && fields.categories.value && fields.start_date && fields.end_date){
                    setLoading(true)
                    schedulePost({
                        ...fields,
                        ...
                        <Col lg={8}>
                            ...
                            <div>
                                ...
                                {loading?(
                                    ...
                                    <button 
                                    onClick={()=>onSubmit()}
                                    className='custom_btns_ift btn btn-primary mt-3'>Publish</button>
                                )}
                                </Col>
                                </Row>
                            </div>
...
export default connect(mapStateProps, {...instaAction,...schedulePostActions,...categoriesActions})(Gallery)
