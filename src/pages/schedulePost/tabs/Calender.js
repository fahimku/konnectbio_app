...
class Calendar extends React.Component {
  ...
  eventClick = (e) => {
    ...
    this.setState({
      ...
      categories:this.props.categories.filter(item=>item.parent_id==e.event._def?.extendedProps?.categories[0]).map(item=>({
                ...
                <Col lg={12}>
                    <div>
                        ...
                        <Input
                            ...
                            name="website"
                            trigger="change"
                            onChange={(e)=>this.setState({redirected_url:e.target.value})}
                            validations="isUrl"
                            validationError={{
                ...
                <Col lg={12}>
                    <div className="date-range-aff">
                        ...
                        <RangePicker
                            ...
                            allowClear={false}
                            value={this.state.addNew?[moment(new Date(this.state.start_date), "YYYY-MM-DD"),moment(new Date(this.state.end_date), "YYYY-MM-DD")]:undefined}
                            ranges={{
                                Today: [moment(), moment()],
                            ...
                            onChange={this.dateRangePickerChanger}
                        />
                        {this.state.submit && !this.state.start_date && !this.state.end_date?(
                                 <small style={{color:'red'}}>Please Fill</small>
                             ):null}
                    </div>
                </Col>
    ...
    if(this.state.showPost){
        if(this.state.showPost){
            if(this.state.showBio){
                if(this.state.publish_date && this.state.redirected_url && this.state.categories.value && this.state.start_date && this.state.end_date){
                    this.setState({loading:true})
                    this.props.updateSchedulePost({
...
onDelete=()=>{
  Swal.fire({
    ...
    if (result.isConfirmed) {
      ...
      this.props.deleteSchedulePost(this.state.media_id)
    .then(()=>{
      this.props.reload()
            ...
            <Widget>
              ...
              <FullCalendar
                ...
                eventContent={(eventInfo) => {
                  return (
                    <div style={{ backgroundColor: 'white' ,paddingBottom:8}} className="pt-2">
                      <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
                        <img src={eventInfo.event.extendedProps.url} width='30%' style={{margin:5}} />
                        <div style={{width:'55%'}}>
                        <p className="mt-2" style={{fontSize:10}}>{eventInfo.event.start.toLocaleTimeString()}</p>
                        <p style={{fontSize:10}}>{eventInfo.event.start.toDateString()}</p>
                        </div>
                      </div>
                    </div>
                  )
        ...
        >
          ...
          <ModalBody className="bg-white affiliate-model image-edit-box p-3">
                    <Row>
                        ...
                        <p style={{color:'gray',overflow:'scroll',overflowX:'hidden',height:'15%'}}>
                        ...
                        <Col lg={8}>
                            ...
                            <div>
                                ...
                                {this.state.loading?(
                                    <button 
                                    className='custom_btns_ift btn btn-primary mt-3'><Loader/></button>
                                ):(
                                    <button 
                                    onClick={()=>this.onSubmit()}
                                    className='custom_btns_ift btn btn-primary mt-3'>Update</button>
                                )}
                              {this.state.deleteLoading?(
                                    ...
                                    className='custom_btns_ift btn btn-primary mt-3'>Delete</button>
                                    )}
                                </Col>
                                </Row>
                            </div>
...
export default connect(mapStateToProps,{...schedulePostActions,...categoriesActions})(Calendar);