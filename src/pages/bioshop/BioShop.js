...
class BioShop extends React.Component {
  constructor(props) {
    ...
    //   this.nextPageInstagramPosts = this.nextPageInstagramPosts.bind(this);
  }
  componentDidMount() {
  ...
  nextPageInstagramPosts = async (username, limit, page) => {
    await axios
      .get(`shop/posts?limit=${limit}&page=${page}&post_type=image,campaign`)
      .then((response) => {
        if (response.data?.message?.result?.hasOwnProperty("next")) {
  ...
  deletePost = async (id) => {
    this.setState({ loading: true });
    await axios.delete(`/posts/remove/${id}`).then(() => {
      let singlePostIndex = this.state.instagramPosts.data.findIndex((item) => item.id === id);
      let currentPost = this.state.singlePost;
        ...
        <Row className="app_main_cont_ift main-container">
          <Col className="left-column" md="5" xs="12" xl="3">
            ...
            <MobilePreview
              ...
              nextPageInstagramPosts={this.nextPageInstagramPosts}
