...
class MyCategory extends React.Component {
  ...
  onSubscribe=(val)=>{
    ...
    return this.props.subscribeServices(val,this.state.priceId,"Category",recurring_payment_type,package_id)
  }
  ...
  fetchMyCategory = async () => {
    await axios
      ...
      .then((response) => {
        ...
        this.setState({ myCategory: selectCategories,categoryLimit:response.data.category_limit });
      })
      .catch((error) => {
        console.log(error);
      });
