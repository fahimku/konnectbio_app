...
export default function Content({ insta, accessToken }) {
    ...
    function onSearch(type) {
        ...
        https://graph.facebook.com/v12.0/ig_hashtag_search?user_id=${insta}&q=${text}&access_token=${accessToken}`)
                ...
                .then(json => {
                    if(json.data){
                        fetch(`https://graph.facebook.com/${json.data[0].id}/${type}?user_id=${insta}&fields=id,caption,media_type,comments_count,like_count,media_url,children,permalink&access_token=${accessToken}`)
                        .then(res2 => res2.json())
                        .then(json2 => {
                            const oembedPromises=json2.data.map((item)=>{
                                return axios.get(`https://graph.facebook.com/v12.0/instagram_oembed?url=${item.permalink}&access_token=${accessToken}`)
                            })
                            Promise.all(oembedPromises).then(res=>{
                               const insightspromises= res.map((item2)=>{
                                    return axios.get(`https://graph.facebook.com/${insta}?fields=business_discovery.username(${item2.data.author_name}){followers_count,media_count}&access_token=${accessToken}`)
                                })
                                Promise.allSettled(insightspromises).then((res3)=>{
                                    ...
                                    const dataUpdate=json2.data.map((item, i) => {
                                        return {
                                            ...item,
            ...
            <Row className="ml-0 mr-0">
              <div className="affiliate_in_col marketing-tabs">
                ...
                >
                  <TabPane tabId="brand">
                    {activeTab === "brand" ? (
                      <HashtagsList next={disableTab} title={"Hashtags"}/>
                    ) : null}
                  </TabPane>
