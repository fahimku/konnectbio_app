...
export const getProfiles = () => async (dispatch) => {
    const res = await axios.post(`${config.hostApi}/v1/graph/monitorProfile/getProfileList`)
    dispatch({
        type: GET_PROFILES,
        payload: res.data
    })
...
export const createProfile = (profileName) => async (dispatch) => {
    const res = axios.post(`${config.hostApi}/v1/graph/monitorProfile/addProfileToList`, {
...
export const searchProfileAc = (userName,check) => async (dispatch) => {
    ...
    const res = await axios.get(`https://graph.facebook.com/${instagram_id}?fields=business_discovery.username(${userName}){followers_count,media_count,profile_picture_url,biography,name,username,website,follows_count,media.limit(300){comments_count,caption,like_count,media_url,permalink,media_type,timestamp,children{id,media_url,media_type,ig_id}}}&access_token=${fb_token}`)
    if(!check)
    dispatch({
        type: SEARCH_PROFILE,
